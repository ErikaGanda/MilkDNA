...
## Objective:
This script will load PMA concentration data, employ a linear model to identify dose dependent effects, and generate visualizations
...
#Read Data
PMAData <- read.table("Inputs/PMAconcentration.txt", sep="\t",  fill = TRUE, header=TRUE)
dim (PMAData)
...
PMAConc.summary <- PMAData %>%
  ...
  summarize(mean_LogCopiesmLmilk=mean(caclLogCopiesmLmilk,na.rm=T),
            ...
            n_total=n())
write.table (PMAConc.summary, "Outputs/PMAConc.summary.txt", sep="\t" ) 
